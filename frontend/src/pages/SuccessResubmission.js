...
const SuccessResubmission = () => {
  ...
  useEffect(() => {
    ...
    const handlePopState = () => {
      navigate("/pendingresubmissions", { replace: true });
    };
  ...
  const handleDownloadPDF = () => {
    ...
    const { submissionId,userId, submittedDate, questions, selectedHierarchy } = submissionData;
    const doc = new jsPDF();
    ...
    const formatAnswer = (q) => {
      ...
      if (q.answerType.includes("duration")) {
        return q.answer.start && q.answer.end
          ? `${new Date(q.answer.start).toLocaleDateString()} to ${new Date(q.answer.end).toLocaleDateString()}`
          : "N/A";
      }
      return q.answer || "N/A";
    };
    ...
    const tableData = questions.map((q, index) => [index + 1, q.question, formatAnswer(q)]);
    ...
    doc.setFontSize(10);
    doc.text("Thank you for your submission!", 105, pageHeight - 20, null, null, "center");
    doc.text("Generated by SmartApproval", 105, pageHeight - 10, null, null, "center");
  ...
  return (
    <div className="success-submission-container">
      ...
      <p className="success-message">Your submission was successfully updated! We appreciate your input.</p>

      <div className="success-details">
        <h2>Submission Details</h2>
        <p><strong>Submission ID:</strong> {submissionData?.submissionId}</p>
        <p><strong>User ID:</strong> {submissionData?.userId}</p>
        <p><strong>Resubmitted Date:</strong> {new Date(submissionData?.submittedDate).toLocaleString()}</p>
      </div>
      ...
      <div className="success-actions">
        ...
        <button className="new-submission-button" onClick={() => navigate("/pendingresubmissions")}>
          🔄 Make a New Resubmission
        </button>
      </div>
    </div>
  );
...
export default SuccessResubmission;
