  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        setIsFinalStep(response.data.isFinalStep);
        setLoading(false);
      ...
      <div className="form-content-container">
        ...
        <div className="form-content-layout">
          <div className="form-content-questions">
            ...
            {questions.length === 0 ? (
              <p className="form-content-no-questions">
              ...
              <ul className="form-content-question-list">
                {questions.map((q, index) => {
                  const shouldHideAnswer =
                    q.hideVisibility &&
                    q.hideVisibility.includes(staff.position);
                  ...
                  return (
                    <li key={index} className="form-content-question-item">
                      ...
                      <div className="form-content-answer">
                        {shouldHideAnswer ? (
                          <div className="form-content-hidden-answer">
                            Answer is not available for your position.
                          </div>
                        ) : (
                          <>
                            {typeof q.answer === "string" &&
                            /\.(pdf|jpg|jpeg|png|gif|docx|xlsx)$/i.test(
                              q.answer
                            ) ? (
                              <div>
                            ...
                            ["single_date", "single_date_future"].includes(
                                q.answerType
                              ) && typeof q.answer === "string" ? (
                              <div>
                                <strong>Answer:</strong> {formatDate(q.answer)}
                              </div>
                            ) : ["duration", "duration_future"].includes(
                                q.answerType
                              ) &&
                              q.answer &&
                              ...
                              <div>
                                ...
                                {formatDate(q.answer.start)}
                                <br />
                                <strong>End Date:</strong>{" "}
                                {formatDate(q.answer.end)}
                              </div>
                            ) : (
                              <div>
          ...
          <div className="form-content-submission-info">
            <div className="form-content-submission-details">
              <h4 className="form-content-submission-title">
